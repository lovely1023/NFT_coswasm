...
const CONTRACT_NAME: &str = "marble-nft";
...
pub fn instantiate(
    ...
    let sub_msg: Vec<SubMsg> = vec![SubMsg {
        msg: WasmMsg::Instantiate {
            ...
            funds: vec![],
            admin: None,
            label: String::from("cw721-base for MarbleNFT"),
        }
        .into(),
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::BuyNative {token_id} => {
            execute_native_buy_move(deps, env, info, token_id, true, None)
        },
        ExecuteMsg::MoveNative {token_id, recipient} => {
            execute_native_buy_move(deps, env, info, Some(token_id), false, Some(recipient))
        },
...
pub fn execute_cw20_buy_move(
    ...
    match msg {
        ReceiveMsg::Buy {token_id, recipient} => {
            ...
            let is_rand = token_id.map_or(false, |v| (true));
            if is_rand {
...
pub fn execute_native_buy_move(
    ...
    let mut sell_id = 0u32;
    let mut price = PRICE.load(deps.storage, sell_id)?;
    let rec_addr:Addr;
    if is_buy {
        let is_rand = token_id.map_or(false, |v| (true));
        if is_rand {
